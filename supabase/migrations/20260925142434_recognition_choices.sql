  ...
  from (
    select
      ...
      jsonb_build_object(
        ...
        'meaning_distractors', case when uc.mastery_level = 1 then (
          select coalesce(jsonb_agg(d.definition_es), '[]'::jsonb) from (
            select n.definition_es from (
              select l2.definition_es from public.lexemes l2
               where l2.id <> l.id and l2.pos = l.pos and l2.definition_es <> l.definition_es
               order by abs(l2.freq_rank - l.freq_rank)
               limit 12
            ) n
            order by random()
            limit 3
